...
class R2Task {
  ...
  async performMint(wallet, amount, logger) {
    try {
      ...
      // Approve USDC
      await this.approvingToken(wallet, this.R2USD_CONTRACT_ADDRESS, this.USDC_CONTRACT_ADDRESS, amountWei, logger);
  ...
  async performStake(wallet, amount, logger) {
    try {
      ...
      await this.approvingToken(wallet, this.sR2USD_CONTRACT_ADDRESS, this.R2USD_CONTRACT_ADDRESS, amountWei, logger);
...
async function performR2Task(logger, privateKeys, proxies, taskConfig, usedNonces) {
  const r2 = new R2Task();
    ...
    try {
      ...
      if (taskConfig.action === "swap") {
        ...
        for (let j = 0; j < taskConfig.swapCount; j++) {
          ...
          const option = r2.generateSwapOption(taskConfig.swapOption, taskConfig.usdcSwapAmount, taskConfig.r2usdSwapAmount);
